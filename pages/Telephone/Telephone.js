...
Page({
 ...
 async handleClick(){
    ...
    if(this.data.phone.length<11)
    {
      ...
      var res = await getRequest({
        url:"http://47.115.15.249:85/consumer/login/sendMessageCode/"+that.data.phone
      })
      console.log(res);
      this.setData({
        isSending:true,
 ...
 // http://47.115.15.249:85/consumer/login/createUser?imageUrl=&&openId=&&tel=&&userName=
    var res = await getRequest({
      url:"http://47.115.15.249:85/consumer/login/createUser?imageUrl="+ifo.avatarUrl+"&&openId="+openid+"&&tel="+that.data.phone+"&&userName="+ifo.nickName
    })
    console.log(res);
  },
  resetBindPhone(){
