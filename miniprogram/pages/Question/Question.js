...
Page({
  ...
  handleTapHottopic(e)
  {
    ...
    this.setData({
      inputValue:this.data.hottopics[e.target.dataset.index]
    })
  },
  //点击发送按钮
  submitQuestion()
  ...
  async getAnswer(question){
    const res = await getRequest(
      {
        url:"http://47.107.110.112/chattingRobot/"+question
      }
      );
  ...
  async getPredict(inputContent)
  {
    const res = await getRequest(
      {
        url:"http://47.107.110.112/chattingRobot/predict/"+inputContent
      }
      );
